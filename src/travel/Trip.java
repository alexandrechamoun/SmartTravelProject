...
/*
The central class of the system. Represents a trip associated with one client, and optionally one transportation and one accommodation.
The ID starts at T2001.
The key method is calculateTotalCost() which adds the base price + transportation cost + accommodation cost using polymorphism.
Also has getTripType() which identifies the trip as a Flight, Train, or Bus trip.
 */
...
public class Trip {
	...
	public Trip(Trip otherTrip) {
		// Generate a brand new ID for this copy.
		// We do NOT copy otherTrip.tripId because the spec says
		// the ID must be newly generated in copy constructors.
		// nextId++ uses the current value then increments it for the next object.
		this.tripId = "T" + nextId++;

		// These three fields are primitives (int, double) or String.
		// They are immutable so copying them directly is safe.
		...
		// Client is an object so we cannot just copy the reference.
		// If we did this.client = otherTrip.client, both trips would point to the same Client object in memory.
		// So we use the Client copy constructor to create a brand new Client.
		// The ternary operator checks if client is null first to avoid
		...
		// At runtime the object is actually a Flight, Train, or Bus.
		// We use instanceof to figure out which concrete type it is,
		// then cast it to that type so we can call the correct copy constructor.
		...
		// Exact same logic as Transportation above.
	...
	public String getTripType() {
		// Check if the transportation field is a (whatever the trip type is) at runtime.
		// instanceof returns true if the object is of that type.
		// We don't need to cast here because we only need the type,
		// not access to Flight-specific fields.
